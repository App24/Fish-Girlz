...
namespace Fish_Girlz.UI{
    public class UICheckbox : UpdatableGUI
    {
        ...
        TextureComponent checkTextureComponent, backgroundTextureComponent;
        ...
        public UICheckbox(Vector2u size, Vector2f position, bool check=false) : base(position)
        {
            ...
            backgroundTextureComponent=AddComponent(new TextureComponent(Utilities.CreateTexture(size, Color.White)));
            uint x=size.X/5;
            uint y=size.Y/5;
        ...
        {
            if(clickComponent.OnClick(new Vector4f(Position, (Vector2f)backgroundTextureComponent.Texture.Size))){
                Checked=!Checked;
            }
